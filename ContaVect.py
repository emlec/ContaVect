...
class Main(object):
    ...
    def _sam_spliter(self):
        ...
        with pysam.Samfile(self.sam, "r") as samfile:
            ...
            LowSize = Sequence(name='LowSize', length=0)
            self.garbage_read = [Secondary, Unmapped, LowMapq, LowSize]

            for read in samfile:
                # Always remove secondary alignments (include supplementary if bwa mem option -M is used)
                if read.is_secondary:
                    Secondary.add_read(read)
                # Filter Unmapped reads
                elif read.is_unmapped:
                    Unmapped.add_read(read)
                # Filter alignments with min_mapq < min_mapq and !=0 (exclude primary alignments)
                elif read.mapping_quality < self.min_mapq and read.mapping_quality != 0:
                    LowMapq.add_read(read)
                # Filter alignments with min_size < min_size
                elif len(read.query_alignment_sequence) < self.min_size:
                    LowSize.add_read(read)
